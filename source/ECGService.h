...
class EcgService {
	...
	*/
    ...
    struct EcgValueBytes {
        ...
        void updateEcgData(int16_t ecgCounter)
        {
            if (ecgCounter <= 127 &&  ecgCounter >= -128) {  //uint8_t = 2^8 different values 
               // valueBytes[FLAGS_BYTE_INDEX] &= ~VALUE_FORMAT_FLAG;
                valueBytes[FLAGS_BYTE_INDEX] = ecgCounter;
            } else {
               // valueBytes[FLAGS_BYTE_INDEX] |= VALUE_FORMAT_FLAG;
                valueBytes[FLAGS_BYTE_INDEX + 1] = (uint8_t)(ecgCounter & 0xFF);
                valueBytes[FLAGS_BYTE_INDEX ] = (uint8_t)(ecgCounter >> 8);
            }
        }
        ...
        {
            if (valueBytes[FLAGS_BYTE_INDEX]) {
