...
const static char DEVICE_NAME[] = "HeartrateXD2";
...
private:
    ...
    void on_init_complete(BLE::InitializationCompleteCallbackContext *params) {
        if (params->error != BLE_ERROR_NONE) {
            EcgUart.printf("Ble initialization failed.");
            return;
        }
    ...
    void start_advertising() {
        ...
        if (error) {
            EcgUart.printf("_ble.gap().setAdvertisingParameters() failed\r\n");
            return;
        }
        ...
        if (error) {
            EcgUart.printf("_ble.gap().setAdvertisingPayload() failed\r\n");
            return;
        }
        ...
        if (error) {
            EcgUart.printf("_ble.gap().startAdvertising() failed\r\n");
            return;
        }
    }
