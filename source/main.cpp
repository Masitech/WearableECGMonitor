...
public:
    ...
    void start(BLE &ble_interface, events::EventQueue &event_queue)
    {
        ...
        EcgUart.printf("Adding demo service\r\n");
        ble_error_t err = _server->addService(_clock_service);

        if (err) {
            EcgUart.printf("Error %u during demo service registration.\r\n", err);
            return;
        }
        ...
        // print the handles
        EcgUart.printf("clock service registered\r\n");
        EcgUart.printf("service handle: %u\r\n", _clock_service.getHandle());
        EcgUart.printf("\thour characteristic value handle %u\r\n", _hour_char.getValueHandle());
        EcgUart.printf("\tminute characteristic value handle %u\r\n", _minute_char.getValueHandle());
...
private:
    ...
    {
        EcgUart.printf("data written:\r\n");
        EcgUart.printf("\tconnection handle: %u\r\n", e->connHandle);
        EcgUart.printf("\tattribute handle: %u", e->handle);
        if (e->handle == _hour_char.getValueHandle()) {
            EcgUart.printf(" (hour characteristic)\r\n");
        } else if (e->handle == _minute_char.getValueHandle()) {
            EcgUart.printf(" (minute characteristic)\r\n");
        } else if (e->handle == _second_char.getValueHandle()) {
            EcgUart.printf(" (second characteristic)\r\n");
        } else {
            EcgUart.printf("\r\n");
        }
        EcgUart.printf("\twrite operation: %u\r\n", e->writeOp);
        EcgUart.printf("\toffset: %u\r\n", e->offset);
        EcgUart.printf("\tlength: %u\r\n", e->len);
        EcgUart.printf("\t data: ");

        for (size_t i = 0; i < e->len; ++i) {
            EcgUart.printf("%02X", e->data[i]);
        }
    ...
    {
        EcgUart.printf("update enabled on handle %d\r\n", handle);
    }
    ...
    {
        EcgUart.printf("update disabled on handle %d\r\n", handle);
    }
    ...
    {
        EcgUart.printf("confirmation received on handle %d\r\n", handle);
    }
    ...
    {
        EcgUart.printf("characteristic %u write authorization\r\n", e->handle);

        if (e->offset != 0) {
            EcgUart.printf("Error invalid offset\r\n");
            e->authorizationReply = AUTH_CALLBACK_REPLY_ATTERR_INVALID_OFFSET;
            return;
        }

        if (e->len != 1) {
            EcgUart.printf("Error invalid len\r\n");
        ...
        if ((e->data[0] >= 60) ||
            ...
            EcgUart.printf("Error invalid data\r\n");
    ...
    void increment_second(void)
    {
        ...
        if (err) {
            EcgUart.printf("read of the second value returned error %u\r\n", err);
            return;
        }
        ...
        if (err) {
            EcgUart.printf("write of the second value returned error %u\r\n", err);
            return;
        }
    ...
    void increment_minute(void)
    {
        ...
        if (err) {
            EcgUart.printf("read of the minute value returned error %u\r\n", err);
            return;
        }
        ...
        if (err) {
            EcgUart.printf("write of the minute value returned error %u\r\n", err);
            return;
        }
    ...
    void increment_hour(void)
    {
        ...
        if (err) {
            EcgUart.printf("read of the hour value returned error %u\r\n", err);
            return;
        }
        ...
        if (err) {
            EcgUart.printf("write of the hour value returned error %u\r\n", err);
            return;
        }
    }
